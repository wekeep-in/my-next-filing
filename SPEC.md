...
- has no unsupported income, deduction, loss, credit, special-rate item, audit requirement, surcharge outside the supported first band, or taxable total income above the applicable ceiling; and
...
- platform-fee GST is evaluated independently once the income character and gross receipts are resolved. It does not stop otherwise supported income tax.

Evaluation never infers classification from a platform brand. An unclear recipient, gross amount, legal relationship, or income character is Unsupported. Ask for explicit reverse-charge status: confirmed no duty, confirmed duty, or Not sure. An old No to “confirmed no reverse charge” is not evidence of a duty and must migrate to Not sure. Unknown fee GST creates independent review, never a favorable turnover-only conclusion.

For an unregistered user with confirmed reverse-charge liability on platform fees, establish registration as required independently of turnover. Use an optional declared liability date within the Tax Year and no later than today to create the existing 30-day registration action. Where turnover also establishes liability, use the earlier date. Unresolved turnover, other compulsory triggers or either applicable date withhold a deadline while preserving the required-registration conclusion. Unknown or earlier-year timing gives an undated prompt to confirm the date; no deadline is invented. For an active normal registration, retain known return periods and add explicit platform-fee reverse-charge payment/reporting guidance. Unknown fee treatment withholds the GST completeness conclusion but preserves known return dates and independent LUT. No GST value, rate, payable amount, ITC, supplier classification or brand inference is calculated.
...
- any foreign asset or signing authority meets the foreign-assets branch below; there is no foreign operation, foreign tax, or treaty-relief fact; and
...
### Foreign assets and signing authority

Ask every user whether, at any time in this Tax Year, they held a foreign asset or financial interest as owner or beneficial owner, or had signing authority over an account outside India. Yes, No and Not sure are explicit choices. Include accounts closed during the year and assets/accounts with no income; ownership or signing authority, not a balance threshold, controls the established filing trigger. A beneficiary-only/trust arrangement, overseas business control, cross-border related-party transaction, transfer-pricing report requirement or unresolved income effect remains outside this branch.

Yes and Not sure reveal an income-scope confirmation. All income-producing work and the practice remain in India; the foreign arrangements create no income, gain, loss, deduction, benefit, foreign tax, relief or foreign operation outside the existing supported inputs. Supported Indian freelance receipts may have passed through an account only where the existing gross-receipt, currency, authorised-route and own-Indian-bank-settlement conditions succeed. Do not collect asset values, balances, foreign currencies, account or institution identifiers, countries, documents or transactions. No/Not sure to the income-scope confirmation stops the estimate. Uncertainty about classification alone, with confirmed income scope, withholds only affected independent conclusions.

An established held-asset/signing-authority branch adds the annual-return trigger under section 263(1)(a)(ix)(A) and (b), including at nil income and nil tax. Reuse the annual-return action and existing non-audit presumptive deadline and completion identity. Unknown asset classification prevents a negative filing conclusion only when no other filing trigger is established. Existing filing triggers, GST and LUT remain independent. An unresolved client account-exposure answer is covered by the established held-asset branch; a No asset answer does not erase a contradictory possible client-account exposure. The user must review that earlier answer.

Show available foreign-asset guidance as a visible Plan card. State that ITR-4 excludes established foreign assets/signing authority under notified rule 164 and that the user must confirm the applicable Tax Year form, reporting period, categories and values from current instructions. This is guidance, not positive return-form selection or schedule preparation. Never reuse a previous Assessment Year's Schedule FA period or infer that a small/zero balance removes disclosure. Keep the existing FEMA transition note visible for foreign clients. No additional invoice-level dates, account-opening permission, retrospective disclosure scheme or penalties are calculated.

Source the income-scope boundary in core Rules, the foreign-asset return trigger in the independent annual-return group, and disclosure/form-exclusion guidance in the independent foreign-guidance group. Missing/stale guidance withholds its card; missing/stale return evidence withholds its filing date and conclusion. Each leaves the otherwise supported calculation available. A domestic-only profile with no foreign-asset or account facts requires no foreign guidance, even if that independent group is stale.

Workspace 9 migrates to 10 in memory. Old profiles with no relevant exclusion and no possible client-account exposure get an absent asset branch. Old supported possible-account profiles retain uncertainty with income scope confirmed only where the existing unsupported-fact declarations excluded additional tax effects; they remain partial until classification is reviewed. Any retained unsupported fact produces uncertain income scope and remains blocked; do not infer a complete income declaration from a previously unsupported Profile. Preserve every original fact, amount, revision, consent and Completion record. Recovery 8 migrates to 9 with both new choices blank. Never auto-clear a legacy foreign-assets checkbox. Reject mixed schemas and hidden confirmations; choosing No clears the income confirmation. Use the existing storage keys and normal validated writes. See [.scratch/foreign-assets-coverage/map.md](.scratch/foreign-assets-coverage/map.md).
...
Deduct the lower of ₹75,000 and aggregate salary once per individual and Tax Year. Add the resulting taxable salary to presumptive income and supported other income. Include actual Indian employer TDS in the existing combined TDS input, counted once. Combined income after the employer NPS deduction controls rebate, marginal relief and the ₹1 crore ceiling; annual-return income triggers use income before that deduction. The presumptive advance-tax date remains 15 March 2027. Return-form selection remains unavailable.
...
If a capital loss remains unused, show its amount and category beside the estimate even when tax is zero. The independently validated annual-return conclusion includes conditional carry-forward guidance. To preserve the option to claim carry-forward, the plan includes the existing annual-return action with explicit conditional wording, including when no other filing trigger is established. Its `required` conclusion then means filing is required to claim carry-forward, not that merely realising a loss unconditionally requires a return. Sections 111, 121 and 263 require timely filing and determination of the loss; do not promise an approved balance. Explain the maximum eight immediately following Tax Years and that ST loss can offset future capital gains but LT loss only future LT gains. Eligible brought-forward losses use the separate branch below; current-year unused losses are not automatically certified or entered there. Stale/missing annual-return evidence withholds its deadline and carry-forward conclusion while preserving core loss set-off, the unused-loss amounts and an explicit guidance-review notice. Preserve the existing conditional advance-tax timing guidance. See the [loss-support plan and research](.scratch/domestic-equity-losses/map.md).

### Eligible brought-forward capital losses

Ask separately whether eligible short-term or long-term capital losses from earlier years remain. No and Not sure are explicit branches; Yes requires a scope confirmation and up to eight rows, one per originating financial year. For Tax Year 2026-27, accepted originating years are 2018-19 through 2025-26. Each row has a year and separate non-negative safe-integer ST and LT remaining balances from records, after previous use and adjustments. Reject duplicate years, future/current years, unsafe sums and unknown fields. Expired or unconfirmed eligibility stops the core estimate; never silently discard a loss.

Require timely filed returns and determined, still-available losses from the supported domestic equity instruments, with no disputes, condonation, ownership transfer, foreign instruments, business/trading losses or unresolved adjustments. Section 536(2)(m) preserves earlier-Act balances with their original restrictions and age; the clock does not restart in 2026. No uploads, return identifiers or automatic loss certification are introduced.

Apply current-year losses first. Then process eligible earlier-year rows oldest first. Within each row, apply LT loss only to remaining LT gains; ST loss to ST gains first and then remaining LT gains. Neither category reduces ordinary income. Show each original balance, every used amount, the remainder and last usable Tax Year. Gains after both stages enter income, rebate, NPS exclusion, filing and surcharge calculations. Earlier-year unused balances stay separate from current-year unused losses and do not create a new eight-year period or a new current-loss filing obligation by themselves. Existing independent filing triggers remain.

Workspace 10 migrates to 11 with no earlier-year losses only where legacy exclusions establish that absence; broad gains/losses or unknown-scope exclusions produce Not sure; retain all legacy exclusions. Replace platform `noRecipientReverseCharge` with explicit `reverseCharge`: old Yes becomes confirmed none; old No or Not sure becomes Not sure. Add an unknown liability date. Recovery 9 migrates to 10 with the new loss branch unanswered and no rows, and the same conservative reverse-charge mapping. Preserve other fields, revisions, consent and Completion records; reject mixed schemas. Deselecting losses clears rows/confirmation; a platform date is retained only for a confirmed duty while unregistered. New eligibility never clears a legacy gains/losses/surcharge stop automatically. See [.scratch/integrated-coverage-upgrades/map.md](.scratch/integrated-coverage-upgrades/map.md).

### Domestic rental income alongside freelancing

Ask Yes, No or Not sure about rental income. Yes requires confirmation of one wholly owned or co-owned Indian residential property with a definite, documented ownership share let for residential use, with annual value and tax-record amounts resolved. Exclude disputed or unascertainable ownership, deemed ownership or clubbing, foreign property, commercial or business-style letting, subletting, self-occupied/deemed-let-out property, unresolved vacancy/unrealised rent, arrears/recovered rent, pre-construction interest and brought-forward property losses. The separate freelance-practice condition still applies; this passive property income does not select or change its presumptive path.

Collect three non-negative safe-integer annual rupee amounts attributable to the user: their tax-record share of established annual value before municipal taxes, their qualifying municipal-tax deduction from taxes actually paid by owners this Tax Year, and their eligible current-year interest deduction. For co-ownership, records must separately establish the ownership share, loan liability and deduction entitlement; do not multiply every amount by a single percentage or enter another owner’s income or deduction. Collect no co-owner identifiers or property address. Interest must relate to the completed let property, be supported by records, payable in India and not deducted elsewhere. Exclude principal, total EMI, personal-loan interest unrelated to the property and pre-construction instalments. Annual value is not assumed to equal bank receipts. Municipal taxes cannot exceed the declared annual value in this slice.
...
Require a separate rental GST confirmation for established coverage: the dwelling and rental supply are in the same state/UT as the existing practice/registration, used only as a residence, the user’s rental share is established as their own supply under their PAN/registration rather than a separate association or entity, and throughout the letting every tenant is either unregistered for GST or a registered sole proprietor renting in their personal capacity for their own residence, on their own account and not the proprietorship concern’s account. The registered-proprietor exception follows Notification 15/2022–Central Tax (Rate). Company leases, business-account rentals and unresolved tenant conditions remain outside this GST branch. Existing No/Not sure answers require explicit review; never change them automatically. This is a product boundary, not a claim that all other rentals are taxable. No/Not sure preserves the income-tax estimate but withholds GST registration/return conclusions and dates pending review. Independent LUT and income-tax actions remain. Require an explicit answer; blank is incomplete. Keep rental GST authority in the independent GST rule groups. Include exempt rental supply value in independently declared all-India GST aggregate turnover; never copy taxable property income or annual value into GST turnover. No GST liability or ITC calculation.
...
Another or uncertain registration state, multiple GSTINs, composition, suspension, cancellation, unsupported reverse charge, or uncertain GST facts makes GST Coverage unavailable when those facts do not change the income path or receipts. If they can change the core tax calculation, the Profile is Unsupported.
...
Support rounded taxable total income after employer NPS and eligible loss set-off up to ₹1 crore, including the supported domestic equity gains. At exactly ₹50 lakh there is no surcharge. Above ₹50 lakh through ₹1 crore, apply 10% surcharge to combined ordinary and supported equity tax after rebate/relief. Reduce surcharge by any excess of tax plus surcharge over tax on ₹50 lakh plus the income exceeding ₹50 lakh, capped at the surcharge. Compute the ₹50 lakh comparison from the same income components: reserve the applicable zero-tax portions, divide ordinary income into its actual marginal slab bands, combine with supported equity-rate bands, and fill the threshold from lower to higher rates. Retain the long-term annual threshold in the comparison. This reproduces the official rate ordering for the supported subset while using current statutory rates and exemptions. Do not hard-code a marginal-relief end point or apply a flat ordinary-slab comparison to mixed income. Keep this surcharge marginal relief separate from the existing rebate-related marginal relief near ₹12 lakh.

Apply cess to income tax plus surcharge after surcharge marginal relief, then apply actual credits/payments and final rounding once. The threshold and ceiling use rounded taxable income after employer NPS; annual-return income tests remain before NPS. Preserve all presumptive receipt/profit, age, residence, new-regime, audit, foreign-income and credit conditions. Positive net equity gains use the same ₹1 crore ceiling. Apply the loss and basic-exemption rules before the mixed-rate surcharge comparison. Income above ₹1 crore remains unsupported.

Show income tax before surcharge, surcharge before marginal relief, surcharge marginal relief, net surcharge and tax including surcharge in the existing breakdown. Briefly explain the higher-income branch beside the estimate and direct users to current return-form/disclosure instructions without selecting a return form. No new Profile amount, choice, storage schema or storage key is needed. Preserve any legacy `surchargeCase` stop until the user reviews and clears it; an old saved/recovered No/Not sure is never changed into a confirmation. The [source review and plan](.scratch/surcharge-and-rental-gst/map.md) establish the boundary.

The Application calculates no surcharge beyond this first surcharge band, deduction other than the supported salary, employer NPS and rental deductions, losses outside the domestic equity branch, special-rate tax other than the domestic equity branch, foreign-tax relief, interest, fee, or penalty.
...
- Regular-books cases, old tax regime, audit cases, surcharge outside the supported first surcharge band, taxable total income above ₹1 crore, and unsupported special-rate income.
