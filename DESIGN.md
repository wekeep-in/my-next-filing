...
Motion is restrained and functional. View transitions use 180ms for groups and 160ms for old/new content. Step entry, disclosure, popover, save, and deletion feedback run between 100ms and 180ms with `cubic-bezier(0.23, 1, 0.32, 1)`. Newly shown warnings, alerts, and field errors enter over 120ms. Pointer presses may scale; keyboard activation does not. `prefers-reduced-motion: reduce` removes movement while preserving immediate state feedback. Confetti is brief, success-only, and suppressed for reduced motion.
