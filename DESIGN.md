...
- The journey rail remains an ordered list with current and completed states. Its mobile wrappers and fixed actions are part of the interaction model, not optional decoration.
- App-wide notices share one context-provided top-bar host with information, warning, destructive, success, and fictional-example variants. They overlay the page's top space without changing document layout. At tablet and mobile widths, reserve the same space above the progress rail even when no notice is present; stacked notices scroll within that space. Notice actions inherit the notice's type size and color.
