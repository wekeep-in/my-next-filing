...
The dedicated `/pitch` route uses a fitted 16:9 canvas with Fraunces headings, Inter labels, semantic colors and one dominant idea per slide. All slides use a solid dark `foreground` background with white text, except the product name on slide seven, which uses a green `primary` background with white text. Slides with a single centered statement use 64px Fraunces type on the 1280px canvas, including the Codex divider. A compact, centered terminology cloud uses Inter text in white, with larger terms progressively bolder and slightly tighter in letter spacing. It keeps tightly packed horizontal and vertical terms, and a 540px by 390px layout scaled uniformly to 110%, with generous surrounding space. A centered mosaic of 25 public tax-portal and help-page screenshots follows it. The mosaic sits on the solid dark `foreground` background and has an irregular outline, square edges, narrow gaps, no shadows or rotation, and varied tile proportions matched to the captured browser viewports. Its 564px by 305px layout scales uniformly to 180% on the slide. The Codex divider uses the dark background and a white Fraunces heading above a centered, dark Magic UI Terminal adaptation. The terminal starts 210px from the top, is 960px wide and 640px high, and extends below the slide, whose bounds clip its lower edge. Its Codex prompt types before sequential green checkmarks appear. Remotion owns the frame clock for pause, replay and still reduced-motion previews. The presentation reuses the recorded cursor overlay with the website tokens. Slides use a simple 180ms opacity fade, except the question on slide six transitions into the product name on slide seven over 1.2 seconds: “When is”, “tax” and the question mark fade away, the remaining words close up and take title case, and the dark surface changes to green while the type stays white. This reveal uses the Remotion frame clock and reduced motion shows the final product title immediately. Other slides retain the simple fade; there are no page turns, drawing filters or animated underlines. Reduced motion skips the fade and opens still frames. Each edited 16:9 recording fits its frame, with the cursor scaled by the same factor and no zoom, pan or cropping. The recordings follow every journey step in order, preserving visible navigation, smooth scrolling, and deliberate reading holds. Each demo plays from a single continuous MP4 at recorded speed; no idle compaction removes reading time. Product recordings retain static money and date values. No narration, music or sound effects play. The presenter view has two columns: scrollable still slide previews on the left and the slide with its controls on the right. The preview rail stays between 168px and 240px wide. The slide workspace fills the remaining width and the height above the control bar, with no fixed aspect ratio, outer padding, border or rounded corners. The complete slide fits within that space without distortion. Letterboxing inherits the active slide background in presenter and fullscreen views, including dark statement and portal slides. A bottom-pinned bar has a top border, matching outlined icon buttons on the left and shadcn Kbd shortcut hints on the right. A wider gap separates slide arrows from playback controls; there is no counter, vertical separator or fullscreen text label in the bar. The preview rail uses the shared ScrollArea with a hover-only scrollbar. There is no page header or header inside the slides. Slides have no footer, event label or slide counter. The privacy slide contains only a centered, four-item list in 48px Fraunces, with local Lucide laptop, unplugged cable, chart and shield icons for browser calculations, no third-party APIs, no web analytics and no financial data uploads, with no heading or supporting copy. A separate statement introduces the privacy reveal after the saved-workspace demo. The Codex slide precedes the final “Built for freelancers. By a freelancer.” statement; there is no closing URL or feedback request. Other text slides contain one large statement or excerpt; the spoken explanation stays only in `.scratch/stage-pitch/content.md`, outside the published site. Demo slides show only the recording in a locally sourced Magic UI Safari mockup, with no slide heading, talking points, caption or footer. These slides and their surrounding presentation space use the existing dark `foreground` token as their background. The Safari frame uses local neutral tokens, and the complete recording fits inside it without cropping. This presentation treatment does not introduce a dark product theme. There is no separate notes panel or app link. Below 900px the previews become a horizontal strip followed by the slide; the control row scrolls horizontally when needed. The canvas scales while accessible controls remain at readable native size.
