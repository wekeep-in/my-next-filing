...
if (expiredCalendar.coverage.gst.kind === 'unavailable') {
  ...
  assert.match(expiredCalendar.coverage.gst.reason, /until we update our rules/)
...
for (const state of statesAndUnionTerritories) {
  ...
  assert.equal(
    result.obligations.filter(({ kind }) => kind === 'gst-qrmp-payment').length,
    8,
  )
  assert.equal(
  ...
  assert.equal(
    result.obligations.find(({ kind }) => kind === 'gst-qrmp-payment')?.dueDate,
    '2026-05-25',
  )
}
...
const expiredMarkup = renderToStaticMarkup(
  createElement(ReviewAreas, { evaluation: expiredExport, onReview: () => {} }),
)
...
for (const group of ['gstCalendar', 'lut'] as const) {
  ...
  assert.equal(
    resultFor(exporter, missing).coverage[group === 'lut' ? 'lut' : 'gst'].kind,
    'unavailable',
  )
}
...
assert.equal(
  assessQuestionnaire(
    {
      draft: { ...draft, gstQuarter3: 'not-sure', gstLutConfirmed: 'not-sure' },
    },
    'gst',
    today,
  ).progression.kind,
...
assert.equal(
  clearInactiveDraft({ ...draft, gstRegisteredFrom: '2026-08-15' }).gstQuarter1,
  '',
)
...
assert.equal(
  deriveWorkspaceView(saved.workspace, { [TAX_YEAR]: monthly }).completedCount,
  1,
)
assert.equal(
...
process.stdout.write(
  'GST calendar, LUT, uncertainty, completion and migration checks passed.\n',
)
