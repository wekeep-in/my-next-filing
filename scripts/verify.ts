...
const professionAtFive = profileFrom({
  ...
  otherIncome: { ...baseCandidate.otherIncome, taxableBankInterest: 0, tds: 0 },
})
...
assert.equal(saveSavedWorkspace(storage, null, workspaceDraft).kind, 'invalid')
assert.equal(deleteSavedWorkspace(storage, null).kind, 'deleted')

process.stdout.write(
  'Profile, evaluation, rules, workspace, and completion checks passed.\n',
)
