...
Every merge proves formatting, lint, tests, and production build behavior. The repository contains the static Cloudflare configuration needed for preview and production deployment without Worker code.

## Work

- Add scripts for `rules:validate`, `test`, `test:watch`, and `test:e2e`.
- Make `pnpm build` run Rule validation, TypeScript, and Vite.
- Configure merge checks in this order: format, lint, Vitest, Playwright Chromium, build.
- Use zero Playwright retries and retain traces or screenshots only on failure.
...
- `pnpm format:check`, `pnpm lint`, `pnpm test`, `pnpm test:e2e`, and `pnpm build` pass from a clean checkout.
...
- Generated test reports, traces, screenshots, videos, and Lighthouse output are ignored rather than committed.
