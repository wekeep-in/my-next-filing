...
| High | The GST result changes to "Review GST registration now" at or above ₹10 lakh/₹20 lakh. | CGST Act section 22 says liability arises when aggregate turnover exceeds, not equals, the applicable threshold. | Keep an at-threshold review nudge only if copy and tests make clear that it is conservative and is not a statement that threshold-based registration is already mandatory. Model the statutory comparison as `turnover > threshold`. |
