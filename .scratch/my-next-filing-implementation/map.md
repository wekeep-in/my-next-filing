...
- Prefer the smallest dependency set and the fewest modules that preserve the three agreed test seams.
...
- [Choose the application and module organization](issues/05-choose-application-organization.md): Use a small seam-first tree with deep Evaluation and Rules modules, flat routes, route-local questionnaire state, one saved-Profile module, and official shadcn components first.
...
- [Reconcile the specification with verified authority](issues/11-reconcile-spec-with-verified-authority.md): `SPEC.md` now contains the approved GST model, corrected advance-tax wording, current primary sources, tutorial review states, and aligned example and test expectations.
- [Choose the test mix and release evidence](issues/06-choose-test-and-release-evidence.md): Use Vitest and Playwright at the three agreed seams, five merge checks, and a manual production-release checklist for law, privacy, Analytics, deployment, performance, and browser evidence.
