  ...
  if (isRecord(value) && value.schemaVersion === 9) {
    const migrateRecord = (record: unknown) => {
      if (
        ...
        Object.hasOwn(record.profile.otherIncome, 'foreignAssets') ||
        !Array.isArray(record.profile.unsupportedFacts)
      )
        return null
      const uncertainIncome = record.profile.unsupportedFacts.length > 0
      const foreign = record.profile.clients.foreign
      const possibleAccount =
        (record.profile.clients.kind === 'foreign' ||
          record.profile.clients.kind === 'mixed') &&
        (!isRecord(foreign) || foreign.accountExposure !== 'none')
      return {
        ...record,
        profile: {
          ...record.profile,
          otherIncome: {
            ...
            foreignAssets:
              uncertainIncome || possibleAccount
                ? {
                    kind: 'possible',
                    incomeConfirmed: uncertainIncome ? 'not-sure' : 'yes',
                  }
                : { kind: 'none' },
          },
        },
  ...
  if (isRecord(value) && value.schemaVersion === 8) {
    const migrateRecord = (record: unknown) => {
      if (
        ...
        !isRecord(record.profile) ||
        !isRecord(record.profile.otherIncome) ||
        Object.hasOwn(record.profile.otherIncome, 'rentalIncome') ||
        !Array.isArray(record.profile.unsupportedFacts)
      )
        return null
      const uncertain = record.profile.unsupportedFacts.some((fact: unknown) =>
        [
          'houseProperty',
      ...
      return {
        ...
        profile: {
          ...
          otherIncome: {
            ...record.profile.otherIncome,
            rentalIncome: { kind: uncertain ? 'not-sure' : 'none' },
          },
        },
      }
    }
  ...
  if (isRecord(value) && value.schemaVersion === 7) {
    const migrateRecord = (record: unknown) => {
      if (
        ...
        !isRecord(record.profile) ||
        !isRecord(record.profile.otherIncome) ||
        !isRecord(record.profile.otherIncome.equityGains) ||
        !Array.isArray(record.profile.unsupportedFacts)
      )
        return null
      const gains = record.profile.otherIncome.equityGains
      if (
        Object.hasOwn(gains, 'shortTermLosses') ||
        Object.hasOwn(gains, 'longTermLosses')
      )
        return null
      const uncertain = record.profile.unsupportedFacts.some((fact: unknown) =>
        [
          ...
          otherIncome: {
            ...
            equityGains:
              gains.kind === 'domestic'
                ? {
                    ...gains,
                    confirmed: uncertain ? 'not-sure' : gains.confirmed,
                    shortTermLosses: 0,
                    longTermLosses: 0,
                  }
                : gains.kind === 'none' && uncertain
                  ? { ...gains, kind: 'not-sure' }
                  : gains,
          },
        },
